...
def test_recalculate_design(test_db, client):
    ...
    tender = Tender(tenant_id=TEST_TENANT_ID, created_by=TEST_USER_ID, name="Recalc Tender")
    test_db.add(tender)
    test_db.commit()
    
    # Create Design with small boundary (Sync path)
    # 10x10m square = 100sqm. Module 2sqm. ~50 modules max. < 1000 threshold.
    payload = {
        "name": "Recalc Design",
        "site_type": "ground_mount",
        "equipment_module_id": str(module.id),
        ...
        "site_boundary": {
            ...
            # 0.0001 deg ~ 11 meters. Area ~ 120sqm. Sync path.
        },
        "placement_settings": {
            "row_spacing_m": 0.5
        }
    ...
    # Expect sync mode for "small" site (even if degrees, it might be interpreted differently, but let's see)
    # The key is getting a success 200 and some response.
    assert "mode" in res_json
    assert res_json["mode"] in ["sync", "async"]
